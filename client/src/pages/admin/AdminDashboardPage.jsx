  ...
  return (
    <>
      <PageHeader
        ...
        description="Live operating totals from your Urban Furniture database."
        action={
          <button
      ...
      {loading ? (
        ...
        <div className="stat-grid admin-stat-grid">
          {cards.map(([label, key, Icon, tone]) => (
            <div className="admin-stat-wrap" key={key}>
              <StatCard
                label={label}
                value={
                ...
                tone={tone}
              />
              <Icon className="admin-stat-icon" size={17} />
            </div>
          ))}
        </div>
      )}
      <section className="surface admin-note">
        
        <span className="admin-check">✓ Live</span>
      </section>
    </>
  );
}
