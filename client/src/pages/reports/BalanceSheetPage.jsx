...
export function BalanceSheetPage() {
  ...
  useEffect(() => {
    ...
    api.get('/reports/balance-sheet', { params: { year }}).then(response => active && setReport(response.data.data)).catch(requestError => active && setError(requestError.response?.data?.message || (requestError.request ? 'The report server is unavailable. Check the API URL in client .env.' : 'Unable to load Balance Sheet report.'))).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [year]);
