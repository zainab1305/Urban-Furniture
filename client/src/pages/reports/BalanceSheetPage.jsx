...
export function BalanceSheetPage() {
  ...
  return <div className="balance-sheet-page">
    ...
    {loading ? <div className="balance-sheet-state">Loading report...</div> : error ? <div className="balance-sheet-state report-error">{error}</div> : <>
      <div className="balance-sheet-report"><section className="balance-sheet-side"><h2>Assets</h2>{rows(report.assets, 'No asset balances')}</section><section className="balance-sheet-side"><h2>Liabilities</h2>{rows(liabilities, 'No liability balances')}</section><div className="balance-sheet-total"><span>Total Assets</span><strong>{money(total(report.assets))}</strong></div><div className="balance-sheet-total"><span>Total Liabilities</span><strong>{money(total(liabilities))}</strong></div></div>
