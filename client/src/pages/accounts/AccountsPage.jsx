...
export function AccountsPage() {
  ...
  return <>
    ...
    {modal && <div className="modal-backdrop"><section className="modal account-modal" role="dialog" aria-modal="true" aria-labelledby="account-modal-title"><button className="modal-close" onClick={() => setModal(null)} aria-label="Close"><X size={17} /></button><div className="eyebrow">CHART OF ACCOUNTS</div><h2 id="account-modal-title">{modal.type === 'create' ? 'New account' : 'Edit account'}</h2><p>Assign a name and accounting classification.</p><form onSubmit={save}><label>Account Name<input name="name" value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} required /></label><label>Type<select name="type" value={form.type} onChange={event => setForm({ ...form, type: event.target.value })}>{Object.entries(typeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setModal(null)}>Cancel</button><button className="primary-button" disabled={saving}>{saving ? 'Saving...' : 'Save account'}</button></div></form></section></div>}
  </>;
}