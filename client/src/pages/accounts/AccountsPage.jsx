...
export function AccountsPage() {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message || "Unable to save account.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const archive = async (account) => {
    ...
    try {
      ...
      setError(
        requestError.response?.data?.message || "Unable to archive account.",
      );
    }
  };
  ...
  return (
    <>
      <PageHeader
        eyebrow="MASTER DATA / CHART OF ACCOUNTS"
        title="Chart of Accounts"
        description="Manage the ledger accounts used to classify every financial transaction."
        action={
      ...
      <section className="surface users-card">
        ...
        {loading ? (
          <div className="empty-table">Loading accounts...</div>
        ) : !accounts.length ? (
          <div className="empty-table">No accounts found.</div>
        ) : (
                  ...
                  <tr key={account.id}>
                    ...
                    <td>
                      <div className="user-actions">
                        <button title="Edit" onClick={() => open(account)}>
                          <Edit3 size={14} />
                        </button>
                        <button
                          title="Archive"
        ...
        <div className="modal-backdrop">
          ...
          >
            ...
            <h2 id="account-modal-title">
              {modal.type === "create" ? "New account" : "Edit account"}
            </h2>
            <p>Assign a name and accounting classification.</p>
            <form onSubmit={save}>
              <label>
                Account Name
                <input
