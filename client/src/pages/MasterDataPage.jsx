...
export function MasterDataPage({ kind }) {
  ...
  const recordCards = records.map(record => <article className={`master-card ${isContact ? 'contact-card' : 'product-card'}`} key={record.id}>
    {isContact && (record.profileImage ? <img className="master-card-avatar" src={record.profileImage} alt="" /> : <div className="master-card-avatar master-card-avatar-fallback">{record.name?.slice(0, 1).toUpperCase()}</div>)}
