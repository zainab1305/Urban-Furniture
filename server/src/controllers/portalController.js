...
export async function dashboard(request, response) {
  ...
  const [invoices, bills, payments] = await Promise.all([
    prisma.salesInvoice.findMany({ where: { customerId: contactId }, include: { paymentAllocations: true }, orderBy: { invoiceDate: 'desc' } }),
    prisma.vendorBill.findMany({ where: { vendorId: contactId }, include: { paymentAllocations: true }, orderBy: { invoiceDate: 'desc' } }),
  ...
  response.json({ success: true, data: {
    invoices: invoices.map(invoice => ({ ...invoice, paid: allocationTotal(invoice.paymentAllocations), outstanding: Math.max(0, numeric(invoice.total) - allocationTotal(invoice.paymentAllocations)) })),
    bills: bills.map(bill => ({ ...bill, paid: allocationTotal(bill.paymentAllocations), outstanding: Math.max(0, numeric(bill.total) - allocationTotal(bill.paymentAllocations)) })),
    payments
  } });
}
...
export async function invoice(request, response) {
  const contactId = requireContact(request, response);
  if (!contactId) return;
  const record = await prisma.salesInvoice.findFirst({ where: { id: request.params.id, customerId: contactId }, include: { paymentAllocations: true, salesOrder: { include: { items: { include: { product: true } } } } } });
  if (!record) return response.status(404).json({ success: false, message: 'Invoice not found.' });
  response.json({ success: true, data: { ...record, paid: allocationTotal(record.paymentAllocations), outstanding: Math.max(0, numeric(record.total) - allocationTotal(record.paymentAllocations)) } });
}

export async function bill(request, response) {
  const contactId = requireContact(request, response);
  if (!contactId) return;
  const record = await prisma.vendorBill.findFirst({ where: { id: request.params.id, vendorId: contactId }, include: { paymentAllocations: true, purchaseOrder: { include: { items: { include: { product: true } } } } } });
...
export async function pay(request, response) {
  ...
  const result = await prisma.$transaction(async transaction => {
    const invoiceRecord = salesInvoiceId ? await transaction.salesInvoice.findFirst({ where: { id: salesInvoiceId, customerId: contactId }, include: { paymentAllocations: true } }) : null;
    const billRecord = vendorBillId ? await transaction.vendorBill.findFirst({ where: { id: vendorBillId, vendorId: contactId }, include: { paymentAllocations: true } }) : null;
    const target = invoiceRecord || billRecord;
    if (!target) throw Object.assign(new Error('Invoice or bill not found.'), { statusCode: 404 });
    const outstanding = numeric(target.total) - allocationTotal(target.paymentAllocations);
